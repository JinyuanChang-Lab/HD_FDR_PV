...
env_sim <- new.env()
load("./S1_plot_nongaussian/S1_simulation_nongaussian.RData", envir = env_sim)
...
get_fdr_matrix <- function(structure, n, p_seq) {
  ...
  for (i in seq_along(p_seq)) {
    p <- p_seq[i]
    r_var <- paste0("r_", struct_prefix, "n", n, "_p", p)
    rgm_var <- paste0("rNgm_", struct_prefix, "n", n, "_p", p)
...
plot_fdr <- function(mat, p_seq, title, save_name) {
  ...
  ggsave(paste0(save_name, ".pdf"), p, device = "pdf", path = "./S1_plot_nongaussian", width = 8, height = 6)
  return(p)
}
