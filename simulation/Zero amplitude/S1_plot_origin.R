...
env_origin <- new.env()
env_sim <- new.env()
load("./S1_plot_origin/S1_simulation_origin.RData", envir = env_origin)
...
get_fdr_matrix <- function(fdr_level, structure, n, p_seq) {
  ...
  for (i in seq_along(p_seq)) {
    ...
    mat[i, ] <- c(colMeans(val_sim)[c(1, 3)], colMeans(val_origin)[c(1, 3)])
  }
  return(mat)
}
...
plot_fdr <- function(mat, p_seq, title, save_name) {
  ...
  ggsave(paste0(save_name, ".pdf"), p, device = "pdf", path = "./S1_plot_origin", width = 8, height = 6)
  return(p)
}
