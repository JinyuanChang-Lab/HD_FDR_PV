...
for (n in n_values) {
  ...
  for (amp in amp_values) {
    for (fdr_cfg in fdr_configs) {
      run_simulation(n, p, k, amp, fdr_cfg$fdr, fdr_cfg$suffix, "K_BH_BBH", "r_O", rho)
    }
  }
}
...
for (n in n_values) {
  ...
  for (amp in amp_values) {
    for (fdr_cfg in fdr_configs) {
      run_simulation(n, p, k, amp, fdr_cfg$fdr, fdr_cfg$suffix, "KBBHw_pre", "r_", rho)
    }
  }
}
...
for (n in n_values) {
  ...
  for (amp in amp_values) {
    for (fdr_cfg in fdr_configs) {
      run_simulation(n, p, k, amp, fdr_cfg$fdr, fdr_cfg$suffix, "GMtest", "rgm_", 2)
    }
  }
}
...
for (n in n_values) {
  ...
  for (amp in amp_values) {
    for (fdr_cfg in fdr_configs) {
      run_simulation(n, p, k, amp, fdr_cfg$fdr, fdr_cfg$suffix, "GMtest_new", "rNgm_", 2)
    }
  }
}

save.image(file = "./S4_simulation.RData")
stopCluster(cl)